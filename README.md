...
```bash
git clone git@github.com:manifoldai/ConuForecast.git
```
...
The runtime for ConuForecast is inside a Docker container. We have a `make` command to launch the appropriate containers.  To launch the docker containers and begin working on a CPU, run from the root directory of the repository:
...
f168e19b8b67        ConuForecast_mlflow            "bash -c 'mlflow ui …"   4 days ago          Up 3 days           127.0.0.1:32770->5000/tcp   ConuForecast_mlflow_<username>
87f03baf686e        ConuForecast_bash     "/bin/bash"              4 days ago          Up 4 days           127.0.0.1:32768->8501/tcp   ConuForecast_bash_<username>
d9bd01600486        ConuForecast_jupyter   "bash -c 'cd /mnt &&…"   4 days ago          Up 3 days           127.0.0.1:32769->8888/tcp   ConuForecast_jupyter_<username>
```
...
```bash
docker exec -it ConuForecast_bash-executer_<username> /bin/bash
```
...
```bash
python ConuForecast/scripts/etl.py configs/examples/config_example.yml
```
...
```bash
python ConuForecast/scripts/evaluate.py configs/examples/config_example.yml
```
