...
PROJECT=ConuForecast
CONTAINER_NAME="ConuForecast_bash_${USER}"  ## Ensure this is the same name as in docker-compose.yml file
...
ci-black: ## Test lint compliance using black. Config in pyproject.toml file
	docker exec $(CONTAINER_NAME) black --check /mnt/ConuForecast

ci-flake8: ## Test lint compliance using flake8. Config in tox.ini file
	docker exec $(CONTAINER_NAME) flake8 /mnt/ConuForecast

ci-test:  ## Runs unit tests using pytest
	docker exec $(CONTAINER_NAME) pytest /mnt/ConuForecast

ci-test-interactive:  ## Runs unit tests using pytest, and gives you an interactive IPDB session at the first failure
	docker exec -it $(CONTAINER_NAME) pytest /mnt/ConuForecast  -x --pdb --pdbcls=IPython.terminal.debugger:Pdb

ci: ci-black ci-flake8 ci-test ## Check black, flake8, and run unit tests
	@echo "CI sucessful"

isort: ## Runs isort to sorts imports
	docker exec $(CONTAINER_NAME) isort -rc /mnt/ConuForecast

black: ## Runs black auto-linter
	docker exec $(CONTAINER_NAME) black /mnt/ConuForecast
...
dev-stop: ## Spin down active containers
	docker-compose -f docker/docker-compose.yml --project-name ConuForecast down
